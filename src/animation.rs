...
//! scalars driven by `ScalarTrack`, the radio-indicator selection
...
/// A scalar driven toward a target under a Cherenkov [`Animation`] on the
/// frame clock. Widget chrome that is not a layer property — interaction
/// state layers, radio choreography, indicator phases — samples its scalars
/// here; transform and opacity are layer properties bound to signals and
/// never pass through a track.
#[derive(Debug)]
struct ScalarTrack {
    current: f32,
    velocity: f64,
...
struct ActiveTrack {
    from: f32,
    /// The velocity the motion starts with, so a retargeted spring stays
    /// continuous.
    velocity: f64,
    to: f32,
    elapsed: Duration,
...
impl ScalarTrack {
    ...
    fn set_target(&mut self, target: f32, animation: Option<Animation>) {
        match animation {
            Some(Animation::Decay(_)) => {
                panic!("hydrolysis: a Decay animation only drives scroll_offset, not a widget scalar")
            }
            Some(animation) => {
    ...
    /// Advances by `delta`; `true` while the track is still moving.
    fn advance(&mut self, delta: Duration) -> bool {
        let Some(active) = &mut self.active else {
            return false;
        };
        active.elapsed += delta;
        let (from, to) = (f64::from(active.from), f64::from(active.to));
        let finished = match &active.animation {
            Animation::Curve(curve) => {
                let t = if curve.duration.is_zero() {
                    1.0
                } else {
                    active.elapsed.as_secs_f64() / curve.duration.as_secs_f64()
                };
                let progress = cherenkov::curve_value(curve, t);
                self.current = (from + (to - from) * progress) as f32;
                t >= 1.0
            }
            Animation::Spring(spring) => {
                let ([position], [velocity]) = cherenkov::spring_step(
                    [from],
                    [active.velocity],
                    [to],
                    spring,
                    active.elapsed.as_secs_f64(),
                );
                self.current = position as f32;
                self.velocity = velocity;
                cherenkov::settled([position], [velocity], [to])
            }
            Animation::Decay(_) => unreachable!("rejected by set_target"),
        };
        if finished {
...
struct AnimatedRadioIndicatorState {
    selected: bool,
    inner_scale: ScalarTrack,
    inner_opacity: ScalarTrack,
    outer_color: ScalarTrack,
    last_tick: Instant,
}
...
impl AnimatedRadioIndicatorState {
    const fn new(selected: bool, now: Instant) -> Self {
        ...
        Self {
            selected,
            inner_scale: ScalarTrack::new(1.0),
            inner_opacity: ScalarTrack::new(selected_progress),
            outer_color: ScalarTrack::new(selected_progress),
            last_tick: now,
        }
    }
...
mod tests {
    ...
    fn scalar_animation_advances_and_stops() {
        ...
        handle.apply_target(
            1.0,
            Some(Animation::Curve(Curve::ease_in_out(Duration::from_millis(120)))),
            start,
        );
    ...
    fn scalar_target_binding_animates_without_snapping() {
        ...
        let first = controller.bind_scalar_target(
            scalar_key(1),
            0.0,
            Animation::Curve(Curve::ease_in_out(Duration::from_millis(1))),
            Instant::now(),
        );
        controller.finish_rebuild_frame_with_inactive_slot_retention(false);
        ...
        let second = controller.bind_scalar_target(
            scalar_key(1),
            1.0,
            Animation::Curve(Curve::ease_in_out(Duration::from_millis(120))),
            start,
        );
        controller.finish_rebuild_frame_with_inactive_slot_retention(false);
    ...
    fn retained_inactive_scalar_slot_keeps_in_flight_target() {
        let mut controller = AnimationController::default();
        let start = Instant::now();
        let animation = Animation::Curve(Curve::ease_in_out(Duration::from_millis(120)));
    ...
    fn radio_indicator_selection_grows_dot_and_fades_color() {
        ...
        let motion = RadioSelectionMotion {
            inner_grow: Animation::Curve(Curve::linear(Duration::from_millis(300))),
            inner_opacity: Animation::Curve(Curve::linear(Duration::from_millis(50))),
            outer_color: Animation::Curve(Curve::linear(Duration::from_millis(50))),
        };
